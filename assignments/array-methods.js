...
// Problem 1
// Who are the top 10 with highest donations?
let sortedDonations = runners.sort((a, b) =>
  a.donation > b.donation ? -1 : 1
);
let largestDonations = sortedDonations.slice(0, 10);

console.log(largestDonations);

// Problem 2
// What's the distribution of shirt sizes?
let twoxlShirts = runners.filter(runner => runner.shirt_size === "2XL");
...
console.log(twoxlShirts.length);
...
let shirtCategories = runners.reduce((acc, runner) => {
  let key = runner.shirt_size;
  acc[key]++;
  return acc;
}, shirtSizes);

console.log(shirtCategories);

// Problem 3
// How much did companies donate?

let companyDonations = runners.reduce((acc, runner) => {
  let key = runner.company_name;
  let value = runner.donation;
  if (key in acc) {
    acc[key] += value;
  } else {
    acc[key] = value;
...
let companyDonations2 = {};

runners.forEach(runner => {
  let key = runner.company_name;
  let value = runner.donation;
  if (key in companyDonations2) {
    companyDonations2[key] += value;
  } else {
    companyDonations2[key] = value;
  }
});
